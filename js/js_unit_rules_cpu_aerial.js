...
function tryCpuAerialGundbitLink(state, total) {
  ensureCpuAerialState(state);

  if (Number(state.aerialGundbitLinkCountThisTurn || 0) >= 3) return false;
  if (Number(state.evade || 0) < 1) return false;

  const ratio = getEvadeRatio(state);
  if (!chance(ratio)) return false;

  reduceEvade(state, 1);
  state.aerialGundbitLinkCountThisTurn += 1;

  total.appendAttacks.push(
    ...createAttack(20, 1, { type: "shoot", beam: true, source: "ガンビット" })
  );
  ...
  return {
    ...
    specials: {
      ...
      special1: {
        ...
        timing: "auto",
        actionType: "auto",
        desc: "回避0で確率無効、回避1以上なら100以上の総合ダメージを最大3回無効化。100ダメージごとに回避倍加。回避割合に応じて、回避1消費で20ダメージのガンビット連携を1ターン最大3回まで追加し、スコアシックス中は赤上限時に強化ターンを延長する。"
      }
    }
  };
}
...
export function onCpuAerialBeforeSlot(state, rolledSlotNumber, context = {}) {
  ...
  if (
    state.formId === "score_six" &&
    Number(state.evade || 0) > Number(state.evadeMax || 0) &&
    Number(state.evade || 0) >= 3
  ) {
    reduceEvade(state, 3);
  ...
  return {
    ...result,
    redraw: result.redraw || messages.length > 0,
    message: messages.join(" / ") || result.message || null
  };
}
...
export function onCpuAerialActionResolved(attacker, defender, context = {}) {
  ...
  if (base.message) total.appendMessages.push(base.message);
  if (Array.isArray(base.appendAttacks)) total.appendAttacks.push(...base.appendAttacks);

  tryCpuAerialGundbitLink(attacker, total);

  if (total.appendAttacks.length > 0) {
    return {
      ...base,
      redraw: true,
      message: total.appendMessages.join(" / ") || null,
      appendAttacks: total.appendAttacks,
      appendAttackLabel: "ガンビット連携"
    };
  }

  return {
    ...base,
    redraw: base.redraw || total.appendMessages.length > 0,
    message: total.appendMessages.join(" / ") || base.message || null
  };
}
...
export function modifyCpuAerialTakenDamage(defender, attacker, attack, damage, context = {}) {
  ...
  if (
    Number(defender.evade || 0) >= 1 &&
    Number(defender.aerialCompositeShieldCount || 0) > 0 &&
    totalDamage >= 100
  ) {
    ...
    return {
      damage: 0,
      cancelled: true,
      message: "CPUエアリアル：コンポジットガンビットシールド 総合ダメージ無効"
    };
  }
...
function resolveCpuAerialExtraSlot(state, slotKey, context = {}, total) {
  ...
  tryCpuAerialGundbitLink(state, total);
}
