...
function ensureGSelfState(state) {
  ...
  INITIAL_UNLOCKED.forEach(pack => {
    state.gselfUnlockedPacks[pack] = true;
  });
  ...
  Object.keys(BASE_EVADE_MAX).forEach(pack => {
    if (typeof state.gselfEvadeMaxBonusByPack[pack] !== "number") {
      state.gselfEvadeMaxBonusByPack[pack] = 0;
    }
  });
...
function addRuleAction(state, amount, context = {}) {
  ...
  if (adapter?.addActionCount && ownerPlayer) {
    return adapter.addActionCount(ownerPlayer, state, value);
  }
...
function addRuleEvade(state, amount, context = {}) {
  ...
  if (adapter?.addEvade && ownerPlayer) {
    return adapter.addEvade(ownerPlayer, state, value);
  }
...
function setEnemyEvadeZero(enemyState, context = {}) {
  ...
  if (adapter?.zeroEvade && enemyPlayer) {
    adapter.zeroEvade(enemyPlayer, enemyState);
    return;
  }
...
function applyPackEvadeMax(state) {
  ...
  state.evadeRedCap = Math.max(
    nextMax,
    Math.min(Number(state.evadeRedCap || nextMax), Number(state.evadeGoldCap || 50))
  );
...
function unlockPack(state, packId) {
  ...
  if (
    (state.gselfUnlockedPacks.high_torque && state.gselfUnlockedPacks.assault) ||
    state.gselfUnlockedPacks.perfect
  ) {
...
function addPackEvadeMax(state, packId) {
  ...
  state.gselfEvadeMaxBonusByPack[packId] =
    Math.max(0, Number(state.gselfEvadeMaxBonusByPack[packId] || 0)) + 1;
...
export function getGSelfDerivedState(state) {
  ...
  if (formId === "space") {
    status.push(`宇宙用シールド:${Math.max(0, Number(state.gselfShieldCounts.space || 0))}/3`);
  }

  if (formId === "atmospheric") {
    status.push(`トワサンガシールド:${Math.max(0, Number(state.gselfShieldCounts.atmospheric || 0))}/3`);
  }

  if (formId === "reflector") {
    status.push(`リフレクター盾:${Math.max(0, Number(state.gselfShieldCounts.reflector || 0))}/3`);
  }

  if (formId === "reflector" || formId === "perfect") {
    status.push(`リフレクター蓄積:${Math.max(0, Number(state.gselfReflectorStockDamage || 0))} / ${Math.max(0, Number(state.gselfReflectorStockCount || 0))}/8`);
  }
  ...
  if (formId === "perfect") {
    ...
    if (state.gselfPhotonSearcherReady) {
      status.push({ text: "フォトン･サーチャー待機", color: "#ffd84a", bold: true });
    }

    if (state.gselfOmniLaserActive) {
      status.push({ text: "slot6:全方位レーザー", color: "#ffd84a", bold: true });
    }
  }

  if (state.gselfRocketBoostCount > 0) {
    status.push(`ロケットブースター:${state.gselfRocketBoostCount}回`);
  }

  if (state.gselfPhotonShieldBarrier > 0) {
    status.push("フォトン･シールド無効:1回");
  }

  const slots = {};
  const reflectorBonus = formId === "reflector" || formId === "perfect"
    ? Number(state.gselfReflectorStockDamage || 0)
    : 0;
  ...
  if (formId === "space" && Number(state.gselfShieldCounts.space || 0) <= 0) {
    slots.slot2 = {
      label: "2EX ビームサーベル最大パワー 100ダメージ",
      desc: "100ダメージ。格闘、ビーム",
      ex: true,
      effect: {
        type: "attack",
  ...
  if (formId === "space" && state.gselfUnlockedPacks.reflector) {
    slots.slot5 = {
      label: "5EX 機動性上昇",
      desc: "宇宙用パックの回避ストック最大値を1増加させる。",
      ex: true,
      effect: {
        type: "custom",
        customType: "gself_evade_max_up_space"
      }
    };
  }
  ...
  if (formId === "atmospheric" && state.gselfUnlockedPacks.tricky) {
    slots.slot5 = {
      label: "5EX 機動性上昇",
      desc: "大気圏パックの回避ストック最大値を1増加させる。",
      ex: true,
      effect: {
        type: "custom",
        customType: "gself_evade_max_up_atmospheric"
      }
    };
  }
  ...
  if (formId === "reflector" && state.gselfUnlockedPacks.assault) {
    slots.slot5 = {
      label: "5EX ビームライフル出力解放 100ダメージ",
      desc: "100ダメージ。射撃、ビーム、軽減不可",
      ex: true,
      effect: {
        type: "attack",
        attackType: "shoot",
  ...
  return {
    status,
    slots,
    specials: buildChangeSpecials(state)
  };
}
...
export function canUseGSelfSpecial(state, specialKey, context = {}) {
  ...
  if (effectType === "gself_space_shield") {
    return {
      allowed: Number(state.gselfShieldCounts.space || 0) > 0,
      message: "使用回数がない"
    };
  }
  ...
  if (effectType === "gself_atmospheric_shield") {
    return {
      allowed: Number(state.gselfShieldCounts.atmospheric || 0) > 0,
      message: "使用回数がない"
    };
  }
  ...
  if (effectType === "gself_reflector_shield") {
    return {
      allowed: Number(state.gselfShieldCounts.reflector || 0) > 0,
      message: "使用回数がない"
    };
  }
  ...
  if (effectType === "gself_photon_armor_shield") {
    return {
      allowed: Number(state.gselfShieldCounts.perfect || 0) > 0,
      message: "使用回数がない"
    };
  }
  ...
  if (effectType === "gself_illusion_funnel") {
    const adapter = getAdapter(context);
    const ownerPlayer = getOwnerPlayer(context);
    const actions = adapter?.getActionCount && ownerPlayer
      ? adapter.getActionCount(ownerPlayer, state)
      : Number(state.actionCount || 0);

    return {
      allowed: actions >= 1,
      message: actions >= 1 ? null : "行動権が足りない"
    };
  }
  ...
  if (effectType === "gself_photon_searcher") {
    ...
    return {
      allowed: ev >= 3,
      message: ev >= 3 ? null : "回避が足りない"
    };
  }

  if (effectType === "gself_high_torque_charge") {
    return {
      allowed: Number(state.hp || 0) > 200,
      message: Number(state.hp || 0) > 200 ? null : "HPが足りない"
    };
  }
  ...
  if (effectType === "gself_change_pack") {
    ...
    return {
      ...
      message: choices.length > 0 ? null : "換装先がない"
    };
  }
  ...
  if (effectType.startsWith("gself_change_")) {
    ...
    return {
      allowed: !!state.gselfUnlockedPacks[packId],
      message: state.gselfUnlockedPacks[packId] ? null : "未解放"
    };
  }
...
export function executeGSelfSpecial(state, specialKey, context = {}) {
  ...
  if (effectType === "gself_space_shield") {
    if (Number(state.gselfShieldCounts.space || 0) <= 0) {
      return { handled: true, redraw: false, message: "使用回数が残っていない" };
    }
  ...
  if (effectType === "gself_atmospheric_shield") {
    if (Number(state.gselfShieldCounts.atmospheric || 0) <= 0) {
      return { handled: true, redraw: false, message: "使用回数が残っていない" };
    }
  ...
  if (effectType === "gself_reflector_shield") {
    if (Number(state.gselfShieldCounts.reflector || 0) <= 0) {
      return { handled: true, redraw: false, message: "使用回数が残っていない" };
    }
  ...
  if (effectType === "gself_photon_armor_shield") {
    if (Number(state.gselfShieldCounts.perfect || 0) <= 0) {
      return { handled: true, redraw: false, message: "使用回数が残っていない" };
    }
  ...
  if (effectType === "gself_high_torque_charge") {
    if (!consumeRuleHp(state, 200, context)) {
  ...
  if (effectType.startsWith("gself_change_")) {
    ...
    return {
      handled: true,
      redraw: true,
      message: ok ? `${PACK_LABEL[packId]}パックへ換装` : "未解放"
    };
  }
...
export function onGSelfBeforeSlot(state) {
  ...
  if (state.gselfPerfectRestTurn > 0) {
    ...
    return {
      redraw: true,
      cancelSlot: true,
      message: "パーフェクトパック反動：このターンは行動不能"
    };
  }
...
export function onGSelfAfterSlotResolved(state, slotNumber, payload = {}) {
  ...
  if (customEffectId === "gself_unlock_reflector") {
    unlockPack(state, "reflector");
    return { redraw: true, message: "リフレクターパック解放" };
  }

  if (customEffectId === "gself_unlock_tricky") {
    unlockPack(state, "tricky");
    return { redraw: true, message: "トリッキーパック解放" };
  }

  if (customEffectId === "gself_unlock_assault") {
    unlockPack(state, "assault");
    return { redraw: true, message: "アサルトパック解放" };
  }

  if (customEffectId === "gself_unlock_high_torque") {
    unlockPack(state, "high_torque");
    return { redraw: true, message: "高トルクパック解放" };
  }

  if (customEffectId === "gself_evade_max_up_space") {
    addPackEvadeMax(state, "space");
    return { redraw: true, message: "宇宙用パック回避上限+1" };
  }

  if (customEffectId === "gself_evade_max_up_atmospheric") {
    addPackEvadeMax(state, "atmospheric");
    return { redraw: true, message: "大気圏パック回避上限+1" };
  }

  if (customEffectId === "gself_rocket_booster") {
    state.gselfRocketBoostCount += 1;
    return { redraw: true, message: "ロケットブースター：次の攻撃アクション威力+50" };
  }

  if (customEffectId === "gself_jammer") {
    setEnemyEvadeZero(enemyState, payload);
    return { redraw: true, message: "ジャマー：相手回避0" };
  }

  if (customEffectId === "gself_traffic_fin") {
    addRuleEvade(state, 2, context);

    return {
      redraw: true,
      appendAttacks: createAttack(10, 5, {
        type: "shoot",
        source: "トラフィック・フィン"
      })
    };
  ...
  if (customEffectId === "gself_omni_laser") {
    setEnemyEvadeZero(enemyState, payload);
    addRuleAction(state, 1, context);

    return {
      redraw: true,
      message: "全方位レーザー：相手回避0＋再行動"
    };
  }
...
export function onGSelfActionResolved(attacker, defender, context = {}) {
  ensureGSelfState(attacker);

  const hitCount = Number(context.hitCount || 0);
  const slotNumber = Number(context.slotNumber || 0);
  const slotLabel = context.slotLabel || "";
  const usedAttack = Number(context.totalCount || 0) > 0 || Array.isArray(context.resolvedAttacks);

  const messages = [];
  let redraw = false;

  if (
    attacker.formId === "perfect" &&
    slotNumber === 6 &&
    slotLabel.includes("フォトン･トルピード") &&
    hitCount > 0
  ) {
    healRuleHp(attacker, hitCount * 50, context);
    reduceEnemyEvadeRule(defender, hitCount * 2, context);
  ...
  if (
    attacker.gselfRocketBoostCount > 0 ||
    attacker.gselfReflectorStockDamage > 0 ||
    attacker.gselfPhotonSearcherReady
  ) {
    attacker.gselfRocketBoostCount = 0;
    attacker.gselfReflectorStockDamage = 0;
    attacker.gselfReflectorStockCount = 0;
    attacker.gselfPhotonSearcherReady = false;

    if (usedAttack) {
      messages.push("Gセルフ攻撃補助を消費");
    }
...
export function modifyGSelfTakenDamage(defender, attacker, attack, damage, context = {}) {
  ...
  if (defender.formId === "perfect" && defender.gselfPhotonArmorShieldActive) {
    if (attack?.beam) {
      const healAmount = Math.floor(getAttackBaseDamage(attack) / 2);
      healRuleHp(defender, healAmount, context);

      return {
        damage: 0,
        cancelled: true,
        message: `フォトン装甲シールド：ビーム無効＋HP${healAmount}回復`
      };
    }

    return {
      damage: Math.floor(Number(damage || 0) / 2),
      message: "フォトン装甲シールド：ダメージ半減"
    };
  }

  if (
    (defender.formId === "reflector" || defender.formId === "perfect") &&
    shouldReflectAttack(attack) &&
    Number(defender.gselfReflectorStockCount || 0) < 8
  ) {
    ...
    return {
      damage: 0,
      cancelled: true,
      message: "リフレクター：射撃ビーム無効化"
    };
  }

  if (defender.gselfAtmosphericShieldActive) {
    return {
      damage: 0,
      cancelled: true,
      message: "シールド(トワサンガ)：攻撃無効"
    };
  }
  ...
  if (
    (defender.formId === "reflector" || defender.formId === "tricky") &&
    attack?.beam &&
    !attack?.ignoreReduction
  ) {
    nextDamage = Math.floor(nextDamage / 2);
...
export function onGSelfDamaged() {
  return {
    redraw: false,
    message: null
  };
...
export function onGSelfEnemyBeforeSlot() {
  return {
    redraw: false,
    message: null
  };
...
export function modifyGSelfEvadeAttempt() {
  return {
    handled: false
  };
}
...
export function onGSelfResolveChoice(state, pendingChoice, selectedValue, context = {}) {
  ...
  if (pendingChoice?.source === "gself_pack_change") {
    ...
    return {
      handled: true,
      redraw: true,
      message: ok ? `${PACK_LABEL[selectedValue]}パックへ換装` : "換装失敗"
    };
  }
