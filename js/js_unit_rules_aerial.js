...
function getAerialConsumeFloor(state) {
  return state?.formId === "score_six" || Number(state?.aerialScoreSixTurns || 0) > 0 ? 10 : 8;
}

function clampAerialRedCapAfterConsume(state) {
  if (!state) return;

  normalizeEvadeCapState(state);

  const floor = Math.max(0, getAerialConsumeFloor(state));
  const current = Math.max(0, Number(state.evade || 0));
  const absoluteMax = Math.max(0, Number(state.overEvadeAbsoluteMax || state.evadeGoldCap || 50));
  const currentCap = Math.max(
    floor,
    Number(state.overEvadeCap || floor),
    Number(state.evadeRedCap || floor)
  );
  ...
  state.overEvadeMode = nextCap > Number(state.evadeMax || 0);
...
function consumeRuleEvade(state, amount, context = {}) {
  ...
  if (adapter?.consumeEvade && ownerPlayer) {
    const ok = adapter.consumeEvade(ownerPlayer, state, cost);
    if (ok) clampAerialRedCapAfterConsume(state);
    return ok;
  }
  ...
  clampAerialRedCapAfterConsume(state);
...
function doubleRuleEvadeRedCap(state, context = {}) {
  ...
  if (
    adapter?.applyToUnifiedPartners &&
    ownerPlayer &&
...
export function canUseAerialSpecial(state, specialKey, context = {}) {
  ...
  if (special.effectType === "aerial_gundbit_link") {
    if (Number(state.aerialGundbitLinkCountThisTurn || 0) >= 3) {
      return { allowed: false, message: "1ターン3回まで" };
    }
  ...
  if (special.effectType === "aerial_data_storm_support") {
    if (state.formId !== "score_six") {
      return { allowed: false, message: "スコアシックス中のみ使用可能" };
    }
...
export function executeAerialSpecial(state, specialKey, context = {}) {
  ...
  if (special.effectType === "aerial_composite_shield") {
    if (Number(state.aerialCompositeShieldCount || 0) <= 0) {
      return {
        handled: true,
        redraw: false,
        message: "使用回数が残っていない"
...
export function onAerialAfterSlotResolved(state, slotNumber, payload = {}) {
  ...
  if (customEffectId === "aerial_bit_on") {
    ...
    return {
      redraw: true,
      message: `ビットオン・エアリアル：軽減+5（合計${state.aerialBitOnReduction}）`
    };
  }
...
export function modifyAerialTakenDamage(defender, attacker, attack, damage, context = {}) {
  ...
  if (reduction > 0) {
    return {
      damage: Math.max(0, Number(damage || 0) - reduction),
      message: `ビットオン軽減：-${reduction}`
    };
  }
...
export function onAerialActionResolved() {
  return { redraw: false, message: null };
}

export function onAerialEnemyBeforeSlot() {
  return { redraw: false, message: null };
}

export function modifyAerialEvadeAttempt() {
  return { handled: false };
}

export function onAerialResolveChoice() {
  return { handled: false, redraw: false, message: null };
}
